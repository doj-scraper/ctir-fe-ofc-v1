...
import { fetchBrands, fetchInventory, fetchModels, type Brand, type InventoryItem, type Model } from "@/lib/api";
...
export default function InventoryPage() {
  ...
  const filteredInventory = useMemo(() => {
    ...
    return inventory.filter((item) => {
      const matchesQuery =
        !query ||
        [item.skuId, item.partName, item.category, item.primaryModel, item.quality]
          .filter(Boolean)
      ...
      if (selectedModel) {
        return (
          item.compatibleModels?.some((model) => model.id === selectedModel) ||
          (item.primaryModel &&
            models.find((model) => model.id === selectedModel)?.marketingName === item.primaryModel)
        );
      }

      if (selectedBrand) {
        const brandModels = models.filter((model) => model.brandId === selectedBrand);
        const brandModelIds = brandModels.map((model) => model.id);
        const brandModelNames = brandModels.map((model) => model.marketingName);

        return (
          item.compatibleModels?.some((model) => brandModelIds.includes(model.id)) ||
  ...
  const selectedModelName = models.find((model) => model.id === selectedModel)?.marketingName;
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_220px_220px_auto]">
              ...
              <div className="space-y-2">
                ...
                <select
                  value={selectedBrand ?? ""}
                  onChange={(event) => {
                    const nextBrand = event.target.value ? Number(event.target.value) : null;
              ...
              <div className="space-y-2">
                ...
                <select
                  value={selectedModel ?? ""}
                  onChange={(event) => setSelectedModel(event.target.value ? Number(event.target.value) : null)}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-left">
                    ...
                    <th className="px-4 py-4 font-semibold text-ct-text">Model</th>
                ...
                <tbody>
                  {filteredInventory.map((item) => (
                    <tr key={item.skuId} className="border-b border-white/5 transition-colors hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-4 text-ct-text-secondary">{item.primaryModel || "Cross-Compatible"}</td>
                      ...
                      <td className="px-4 py-4 text-ct-text-secondary">{item.quality || "Standard"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
