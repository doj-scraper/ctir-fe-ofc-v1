...
Frontend calls POST /api/checkout (via createCheckout() in lib/api.ts:327–345)
Backend creates a Stripe PaymentIntent, returns clientSecret in response
Frontend uses @stripe/stripe-js / Stripe Elements to complete payment in the browser
2.2 Where Supabase User IDs Need to Link to Stripe Customers
After Phase 1 migrates auth, the backend's POST /api/webhooks/clerk Clerk webhook (backend-status-report0325.md:96) must be replaced with a Supabase Auth webhook that creates/updates a User row in Postgres with the Supabase user.id. The frontend's lib/api.ts:401–411 (UserProfile.clerkId) will need to become supabaseId.
...
prisma/schema.prisma → lives in the backend repo (CellTech/Test/schema.prisma per the original project layout, now at celltech-backend/prisma/schema.prisma per backend-status-report0325.md:107)
...
Backend .env (for reference — from DOCS/project docs/BACKENDREPORT.md:851-856)
Code
...
Checks if NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY and CLERK_SECRET_KEY are set (line 3–5)
If not set, passes through all requests with NextResponse.next() (line 8–10)
Dynamically imports clerkMiddleware and createRouteMatcher (line 12)
Protects /dashboard(.*) and /admin(.*) routes with auth.protect() (lines 14–23)
After Phase 1 (Supabase), middleware.ts must:

Create a Supabase SSR client using createMiddlewareClient()
Call supabase.auth.getSession() to refresh the session cookie
Redirect unauthenticated users away from /dashboard and /admin
Forward the supabase response cookies
...
.squad/skills/optional-cache-auth/SKILL.md: describes the pattern of optional Redis blacklist checking in celltech-backend/src/middleware/auth.ts
